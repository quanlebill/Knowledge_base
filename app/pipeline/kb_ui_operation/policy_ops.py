...
_DB_TYPE = {
    "natural_language": PolicyFilteringType.NATURAL_LANG.value,
    "exact_word":       PolicyFilteringType.EXACT_MATCH.value,
}
_UI_TYPE = {v: k for k, v in _DB_TYPE.items()}


def _to_filter_policy(row: dict) -> dict:
    fmt = row.get("configformat", "")
    ui_type = _UI_TYPE.get(fmt, "natural_language")
    cfg = row.get("config") or {}
    rules: list = cfg.get("rules") or []
    content = json.dumps(rules) if ui_type == "exact_word" else (rules[0] if rules else "")
    return {
...
async def create_filter_policy(
    ...
    resp = await postgres.insert(KBFilterPolicyInsert(
        tenant_id=tenant_id,
        policy_name=name,
        configformat=PolicyFilteringType(_DB_TYPE[ptype]),
...
async def update_filter_policy(postgres, policy_id: str, body: dict) -> dict:
    ...
    if "type" in body and "content" in body:
        ...
        updates["configformat"] = _DB_TYPE.get(ui_type, PolicyFilteringType.NATURAL_LANG.value)
        updates["config"] = {"rules": _rules_from_body(ui_type, content)}
    ...
    async with postgres.get_client() as session:
        await session.execute(
            ...
            .where(KBFilterPolicyORM.policy_id == policy_id)
            .values(**updates)
        )
    ...
    if resp.code == 200 and resp.data:
        ...
        async with postgres.get_client() as session:
            await session.execute(
                ...
                .where(KBExtractionPolicyORM.policy_id == pid)

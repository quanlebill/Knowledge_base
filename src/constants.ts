...
export interface SubNavItem {
  ...
  badge?: number | string;
  comingSoon?: boolean;
  action?: boolean;       // triggers an action (e.g. open wizard) instead of sub-tab navigation
  permission?: string;    // if set, only show when the current role has this permission
}
...
export const MODULE_SUB_ITEMS: Record<string, SubNavItem[]> = {
  ...
  'knowledge-ops': [
    ...
    { id: 'ACTION_INGEST',    label: 'Add Data',      icon: Plus,      action: true, permission: 'add_data' },
    { id: 'ACTION_WAREHOUSE', label: 'Add Warehouse', icon: ServerCog, action: true, permission: 'add_warehouse' },
  ],

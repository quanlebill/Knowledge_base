...
const SecondaryNav = () => {
  ...
  const navItems    = subItems.filter((i: SubNavItem) => !i.action);
  const actionItems = subItems.filter((i: SubNavItem) => i.action);
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar py-2 px-2">
        ...
        {actionItems.length > 0 && (
          <div className="mt-3 pt-3 border-t border-[#D6C79F]/60">
            <p className="px-2 pb-1.5 text-[9px] font-black text-[#5A4209]/50 uppercase tracking-widest">Quick Add</p>
            <div className="space-y-1.5">
              {actionItems.map((item: SubNavItem) => (
                <button
                  key={item.id}
                  onClick={() => setPendingAction(item.id)}
                  className="w-full flex items-center gap-2.5 px-3 py-2.5 bg-[#1C1200] border border-[#8A5A00]/40 rounded-xl text-left hover:bg-[#2A1A00] transition-all cursor-pointer"
                >
                  <item.icon className="w-3.5 h-3.5 text-[#D9B86C] shrink-0" />
                  <span className="text-[11px] font-bold text-[#D9B86C] truncate">{item.label}</span>
                </button>
              ))}
            </div>
          </div>
...
const MobileDrawer = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  const { role, industry, user, activeModule, navigate, subTab, setSubTab, setPendingAction } = useAppState();
  const { logout } = useAuth();
            ...
            <div className="flex-1 overflow-y-auto py-3 px-3 space-y-3">
              ...
              {subItems.length > 0 && (
                <div>
                  ...
                  <div className="space-y-0.5">
                    {subItems.filter((i: SubNavItem) => !i.action).map((item: SubNavItem) => (
                      <button
                        key={item.id}
                  ...
                  {subItems.some((i: SubNavItem) => i.action) && (
                    <div className="mt-2 space-y-1">
                      {subItems.filter((i: SubNavItem) => i.action).map((item: SubNavItem) => (
                        <button
                          key={item.id}
                          onClick={() => { setPendingAction(item.id); onClose(); }}
                          className="w-full flex items-center gap-2.5 px-3 py-2.5 bg-[#1C1200] border border-[#8A5A00]/40 rounded-xl text-left hover:bg-[#2A1A00] transition-all cursor-pointer"
                        >
                          <item.icon className="w-3.5 h-3.5 text-[#D9B86C] shrink-0" />
                          <span className="text-[11px] font-bold text-[#D9B86C]">{item.label}</span>
                        </button>
                      ))}
                    </div>
                  )}

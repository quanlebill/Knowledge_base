...
const SIZE_ALIAS: Record<'sm' | 'md' | 'lg' | 'xl' | 'full', keyof typeof SIZE_CONFIG> = {
  sm: 'compact', md: 'standard', lg: 'wide', xl: 'xwide', full: 'workspace',
};

const resolveSize = (s: DrawerSizeKey): keyof typeof SIZE_CONFIG =>
  (SIZE_ALIAS as any)[s] ?? (s as keyof typeof SIZE_CONFIG);
...
export const DetailDrawer = ({
  ...
  const startResize = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      setWidth(Math.max(sizeRange.min, Math.min(maxAllowed, startWidth + delta)));
    };
    const onUp = () => {
  ...
  return (
    <>
      ...
      >
        ...
        {/* ── Body ── */}
        <div className={cn(
          'drawer-body flex-1 overflow-x-hidden',
          fixedHeight ? 'flex flex-col overflow-hidden' : 'overflow-y-auto custom-scrollbar',
        )}>
          {children}
        </div>

        {/* ── Footer ── (null = hide entirely, undefined = default History/Audit) */}
        {footer !== null && (
          <div className="drawer-footer px-6 py-3.5 flex items-center justify-between shrink-0 gap-3 flex-wrap">
            <div className="flex gap-2 flex-wrap">
              {footer ?? (
                <>
                  <button className="btn-secondary"><History className="w-3.5 h-3.5" /> History</button>
                  <button className="btn-secondary"><Shield className="w-3.5 h-3.5" /> Audit</button>
                </>
              )}
            </div>
            {footer === undefined && (

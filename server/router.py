...
def _ok(res) -> Any:
    """Unwrap a ResponseModel; raise ValueError on error so handlers can catch it."""
    if res.code >= 400:
...
def _policy_rules(policy_type: str, content: str) -> list:
...
def _map_doc(row: dict) -> dict:
    ...
    return {
        ...
        "version":     "v1.0",
        "author":      meta.get("author") or _s(row.get("added_by")),
        "lastUpdated": _s(row.get("added_on", "")),
        "metadata": {
            "type":          doc_type,
...
async def get_fleet_stats(claims: JWTClaims = Depends(parse_jwt)):
    try:
        ...
        gold_docs = _ok(await pg.read("KBData", {
        ...
        qdrant_rows = _ok(await pg.read_join({
        ...
        # Neo4j totals
        neo_conns = _ok(await pg.read("KBNeo4jConnection", {"tenant_id": claims.tenant_id}))
        ...
        batches = _ok(await pg.read("KBConflictBatch", {"tenant_id": claims.tenant_id, "limit": 500}))
...
async def get_documents(claims: JWTClaims = Depends(parse_jwt)):
    ...
    try:
        rows = _ok(await pg.read("KBData", {"tenant_id": claims.tenant_id, "limit": 200}))
...
async def update_document(
    ...
    try:
        ...
        _ok(await pg.update("KBData", update))
...
async def delete_document(doc_id: str, claims: JWTClaims = Depends(parse_jwt)):
    try:
        _ok(await pg.delete("KBData", {"data_id": uuid.UUID(doc_id)}))
        return OK({"doc_id": doc_id})
    except Exception as e:
        return ERR(500, str(e))
...
async def get_kg_documents(claims: JWTClaims = Depends(parse_jwt)):
    """Gold-layer documents shown in the DATABASE tab."""
    try:
        rows = _ok(await pg.read("KBData", {
            "tenant_id":    claims.tenant_id,
            "current_tier": "gold",
            "limit":        200,
        }))
...
async def get_chunks(doc_id: str, claims: JWTClaims = Depends(parse_jwt)):
    """
    JOIN KBTextBlock ← KBTextBlockVersion to return all chunks with their
    full version history in a single query.
    UI fields: id, title, text (active version), versions[].
    """
    try:
        rows = _ok(await pg.read_join({
...
async def create_chunk_version(
    ...
    try:
        ...
        existing = _ok(await pg.read("KBTextBlockVersion", {"block_id": chunk_uuid}))
        next_num  = max((v["version_number"] for v in existing), default=0) + 1
        res = _ok(await pg.create("KBTextBlockVersion", {
            "block_id":       chunk_uuid,
...
async def activate_chunk_version(
    ...
    try:
        chunk_uuid = uuid.UUID(chunk_id)
        versions = _ok(await pg.read("KBTextBlockVersion", {"block_id": chunk_uuid}))
        target = next(
        ...
        _ok(await pg.update("KBTextBlockVersion", {
            "version_id": target["version_id"],
            "is_active":  True,
        }))
        return OK({"version_id": _s(target["version_id"])})
    except Exception as e:
        return ERR(500, str(e))
...
async def delete_chunk_version(
    ...
    try:
        versions = _ok(await pg.read("KBTextBlockVersion", {"block_id": uuid.UUID(chunk_id)}))
        ...
        _ok(await pg.delete("KBTextBlockVersion", {"version_id": target["version_id"]}))
        return OK({"version_id": _s(target["version_id"])})
    except Exception as e:
        return ERR(500, str(e))
...
async def delete_chunk(doc_id: str, chunk_id: str, claims: JWTClaims = Depends(parse_jwt)):
    try:
        _ok(await pg.delete("KBTextBlock", {"block_id": uuid.UUID(chunk_id)}))
...
async def update_table_row(
    ...
    try:
        records = _ok(await pg.read("KBTextTable", {"version_id": uuid.UUID(table_id)}))
        if not records:
        ...
        data["rows"] = rows
        _ok(await pg.update("KBTextTable", {"version_id": uuid.UUID(table_id), "data": data}))
...
async def delete_table(doc_id: str, table_id: str, claims: JWTClaims = Depends(parse_jwt)):
    try:
        _ok(await pg.delete("KBTextTable", {"version_id": uuid.UUID(table_id)}))
...
async def get_doc_configs(doc_id: str, claims: JWTClaims = Depends(parse_jwt)):
    try:
        doc_rows = _ok(await pg.read("KBData", {
            "tenant_id": claims.tenant_id,
            "data_id":   uuid.UUID(doc_id),
            "limit":     1,
        }))
...
async def activate_doc_config(doc_id: str, config_id: str, claims: JWTClaims = Depends(parse_jwt)):
    try:
        _ok(await pg.update("KBWarehouse_Config", {
...
async def _resolve_warehouse_id(tenant_id: uuid.UUID, candidate: uuid.UUID) -> uuid.UUID:
    ...
    """
    try:
        rows = _ok(await pg.read("KBData", {"tenant_id": tenant_id, "data_id": candidate, "limit": 1}))
        if rows:
...
async def _get_warehouse_configs(warehouse_id: uuid.UUID) -> ResponseModel:
    ...
    rows = _ok(await pg.read("KBWarehouse_Config", {"warehouse_id": warehouse_id}))
    result = []
    for r in rows:
        cfg = r.get("config") or {}
        result.append({
...
async def _create_warehouse_config(
    ...
    res = _ok(await pg.create("KBWarehouse_Config", {
...
async def delete_warehouse_config(
    ...
    try:
        _ok(await pg.delete("KBWarehouse_Config", {"config_id": uuid.UUID(config_id)}))
        return OK({"config_id": config_id})
    except Exception as e:
        return ERR(500, str(e))
...
async def delete_warehouse_config_table(
    ...
    try:
        ...
        resolved = await _resolve_warehouse_id(claims.tenant_id, uuid.UUID(warehouse_id))
        records = _ok(await pg.read("KBWarehouse_Config", {
...
async def toggle_qdrant_collection(
    ...
    try:
        _ok(await pg.update("KBQdrantCollection", {
...
async def get_neo4j_graph(claims: JWTClaims = Depends(parse_jwt)):
    ...
    try:
        ...
        conn_rows = _ok(await pg.read("KBNeo4jConnection", {"tenant_id": claims.tenant_id}))
        if not conn_rows:
            return OK({"nodes": [], "edges": []})
        conn_id = conn_rows[0]["connection_id"]

        # Nodes from Postgres registry
        node_rows = _ok(await pg.read("KBNeo4jNode", {"connection_id": conn_id, "limit": 500}))
        node_id_set = {_s(n["node_id"]) for n in node_rows}

        # Edges — join KBNeo4jNode (FROM side) to KBNeo4jRelationship
        edge_rows = _ok(await pg.read_join({
        ...
        return OK({
            ...
            "edges": [
                ...
                for e in edge_rows
                if _s(e.get("to_id")) in node_id_set  # guard: to-node belongs to tenant
            ],
        })
    except Exception as e:
        return ERR(500, str(e))
...
async def resolve_conflict(
    ...
    try:
        ...
        _ok(await pg.update("KBConflict", {
...
async def get_filter_policies(claims: JWTClaims = Depends(parse_jwt)):
    try:
        rows = _ok(await pg.read("KBFilterPolicy", {"tenant_id": claims.tenant_id, "limit": 100}))
        return OK([
            {
                "id":         _s(r["policy_id"]),
                "name":       r.get("policy_name"),
                "type":       _policy_fmt_to_type(r.get("configformat") or ""),
                "content":    _policy_content(r.get("configformat") or "", (r.get("config") or {}).get("rules", [])),
                "added_by":   _s(r.get("created_by", "")) or "platform-admin",
...
async def create_filter_policy(
    ...
    try:
        res = _ok(await pg.create("KBFilterPolicy", {
            "tenant_id":    claims.tenant_id,
            "policy_name":  body.name,
            "configformat": _policy_type_to_fmt(body.type),
            "is_active":    True,
            "created_by":   claims.user_id,
            "config":       {"rules": _policy_rules(body.type, body.content), "threshold": 0.8},
        }))
...
async def get_filter_policy(policy_id: str, claims: JWTClaims = Depends(parse_jwt)):
    try:
        ...
        return OK({
            "id":         _s(match["policy_id"]),
            "name":       match.get("policy_name"),
            "type":       _policy_fmt_to_type(fmt),
            "content":    _policy_content(fmt, cfg.get("rules", [])),
            "added_by":   _s(match.get("created_by", "")) or "platform-admin",
...
async def update_filter_policy(
    ...
    try:
        ...
        if "type" in data:
            update["configformat"] = _policy_type_to_fmt(data["type"])
        if "active" in data:
            update["is_active"] = data["active"]
        if "content" in data:
            t = data.get("type", "natural_language")
            update["config"] = {"rules": _policy_rules(t, data["content"]), "threshold": 0.8}
        _ok(await pg.update("KBFilterPolicy", update))
...
async def delete_filter_policy(policy_id: str, claims: JWTClaims = Depends(parse_jwt)):
    try:
        _ok(await pg.delete("KBFilterPolicy", {"policy_id": uuid.UUID(policy_id)}))
        return OK({"policy_id": policy_id})
    except Exception as e:
        return ERR(500, str(e))
...
async def get_extraction_policy(claims: JWTClaims = Depends(parse_jwt)):
    try:
        rows = _ok(await pg.read("KBExtractionPolicy", {"tenant_id": claims.tenant_id, "limit": 5}))
...
async def update_extraction_policy_custom(
    ...
    try:
        ...
        _ok(await pg.update("KBExtractionPolicy", {
            "policy_id":       rows[0]["policy_id"],
            "custom_override": body.custom,
        }))
        return OK({"policy_id": _s(rows[0]["policy_id"])})
    except Exception as e:
        return ERR(500, str(e))

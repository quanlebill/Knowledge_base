...
CREATE TABLE IF NOT EXISTS pipelines (
  ...
  triggered_by    TEXT NOT NULL,                        -- username (Keycloak sub)
  trigger_type    TEXT DEFAULT 'MANUAL',                -- MANUAL / GIT_PUSH / SCHEDULED
  commit_sha      TEXT,
  branch          TEXT,
  package_version TEXT,
  status          TEXT NOT NULL DEFAULT 'PENDING',      -- PENDING / RUNNING / BUILDING /
                                                        -- SCANNING / AWAITING_APPROVAL /
...
CREATE TABLE IF NOT EXISTS release_packages (
  ...
  artifact_paths    JSONB DEFAULT '[]',                 -- MinIO / S3 paths
  validation_score  SMALLINT,                           -- 0–100
  status            TEXT NOT NULL DEFAULT 'BUILDING',   -- BUILDING / SCAN_PENDING / SCAN_FAILED /
                                                        -- VALIDATED / PROMOTING / PROMOTED / REJECTED
  created_by        TEXT NOT NULL,
  environment_targets JSONB DEFAULT '[]',               -- ["dev","staging","production"]
  scan_result       JSONB DEFAULT '{}',                 -- trivy, bandit, pip_audit results
...
CREATE TABLE IF NOT EXISTS rollback_operations (
  ...
  triggered_by    TEXT NOT NULL,
  reason          TEXT,
  status          TEXT NOT NULL DEFAULT 'INITIATED',    -- INITIATED / STEP1_DB / STEP2_KONG /
                                                        -- STEP3_DEPLOY / SUCCESS /
                                                        -- PARTIAL_ROLLBACK / FAILED
